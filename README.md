...
- **Disk-based state machine** — All progress tracked in `state.json` via a 26-subcommand CLI (32 MCP tools including `search_patterns`, `subscribe_events`, `ast_summary`, and `ast_find_definition`); pipelines survive context compaction and session restarts
