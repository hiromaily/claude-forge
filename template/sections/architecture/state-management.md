...
All pipeline state is managed through the **Go MCP server** (`forge-state`), which exposes 44 typed tool calls. State is persisted in `state.json` within the workspace directory.
...
The Go MCP server exposes **44 typed tool calls** across 8 categories:
