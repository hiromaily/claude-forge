...
- **MCP サーバ** — Go バイナリ（`forge-state`、47 ツール）はそのまま動作。登録形式のみ変更（`.codex-plugin/plugin.json` の `mcpServers` エントリまたは `~/.codex/config.toml` の `[mcp_servers.*]`）
