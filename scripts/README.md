...
| [`validate-input.sh`](validate-input.sh) | **DEPRECATED — use `mcp__forge-state__validate_input` MCP tool instead. Kept as fallback for non-MCP flows.** Deterministic input validation. Checks for empty input, too-short descriptions (< 5 chars after flag stripping), flags-only input, and URL format validation (GitHub Issue, Jira Issue). Exit 0 = valid, exit 1 = invalid. | SKILL.md orchestrator (via Bash tool, before Workspace Setup) |
...
| [`test-hooks.sh`](test-hooks.sh) | Automated test suite. Covers all state-manager commands, validate-input.sh smoke tests, pre-tool-hook rules (read-only, commit blocking, main/master checkout block), post-agent-hook validation, stop-hook behavior, common.sh sourcing smoke tests, and edge cases (abandoned pipelines, special characters, numeric type preservation) — run to see current count. | Manual (`bash scripts/test-hooks.sh`) |
