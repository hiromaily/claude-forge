...
| [`query-specs-index.sh`](query-specs-index.sh) | Keyword-score matching against `.specs/index.json`. Reads the index built by `build-specs-index.sh`, scores past pipeline entries by taskType match (+2) and keyword overlap (+1 per word), and emits a formatted markdown block (`## Past Review Feedback (from similar pipelines)`) with up to 3 top-scoring entries to stdout. Returns empty stdout (exit 0) when the index is absent, empty, or no entries score >= 2. | SKILL.md orchestrator (via Bash tool, Phase 3 before architect agent and Phase 4 before task-decomposer agent) |
