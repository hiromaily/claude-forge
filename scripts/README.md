...
| [`validate-input.sh`](validate-input.sh) | **DEPRECATED — use `mcp__forge-state__validate_input` MCP tool instead. Kept as fallback for non-MCP flows.** Deterministic input validation. Checks for empty input, too-short descriptions (< 5 chars after flag stripping), flags-only input, and URL format validation (GitHub Issue, Jira Issue). Writes a timestamp marker on success for pre-tool-hook's init guard. Exit 0 = valid, exit 1 = invalid. | SKILL.md orchestrator (via Bash tool, before Workspace Setup) |
