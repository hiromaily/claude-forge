...
| [`pre-tool-hook.sh`](pre-tool-hook.sh) | PreToolUse hook. Enforces four rules: Rule 1 (blocks Edit/Write on source files during Phase 1-2 read-only enforcement, with a workspace carve-out for artifact writes), Rule 2 (blocks `git commit` during parallel Phase 5 tasks for batch-commit enforcement), Rule 3f (warns when `phase_start phase-1` is called without effort set — non-blocking), Rule 5 (blocks `git checkout/switch` to `main` or `master` during an active pipeline). Sources `scripts/common.sh` for `find_active_workspace`. Fail-open: allows action if no active pipeline or jq unavailable. | Claude Code (automatic, via hooks.json) |
