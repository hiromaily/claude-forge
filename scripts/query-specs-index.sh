#!/usr/bin/env bash
# query-specs-index.sh — Query .specs/index.json for past review feedback relevant to current pipeline
#
# Usage: bash scripts/query-specs-index.sh <workspace> [task_type]
#
...
#     ## Past Review Feedback (from similar pipelines)
#     ...
#
# Exit codes:
...
if [ -z "${WORKSPACE}" ]; then
  die "Usage: query-specs-index.sh <workspace> [task_type]"
fi
...
# --- Score and filter entries via single jq invocation ---
#
# Scoring:
