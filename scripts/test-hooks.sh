...
# ============================================================
# validate-input.sh smoke tests
# ============================================================

echo ""
echo "=== validate-input.sh smoke tests ==="

VI="${SCRIPT_DIR}/validate-input.sh"

echo ""
echo "--- smoke: empty input is rejected ---"
VI_EXIT=0
VI_STDERR="$(bash "$VI" "" 2>&1 >/dev/null)" || VI_EXIT=$?
if [ "$VI_EXIT" -ne 0 ]; then
  pass "validate-input.sh: empty input rejected (exit non-zero)"
else
  fail "validate-input.sh: empty input rejected (exit non-zero)" "expected non-zero exit, got $VI_EXIT"
fi

echo ""
echo "--- smoke: valid plain text input is accepted ---"
VI_EXIT=0
bash "$VI" "Add retry logic to the API client" >/dev/null 2>&1 || VI_EXIT=$?
if [ "$VI_EXIT" -eq 0 ]; then
  pass "validate-input.sh: valid plain text accepted (exit 0)"
else
  fail "validate-input.sh: valid plain text accepted (exit 0)" "expected exit 0, got $VI_EXIT"
fi
